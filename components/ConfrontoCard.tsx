...
const ConfrontoCard: React.FC<ConfrontoCardProps> = ({ confronto, teams }) => {
  ...
  const getWinnerClass = (teamScore: number, otherTeamScore: number) => {
    if (teamScore > otherTeamScore) return 'text-emerald-500 dark:text-emerald-400';
    if (teamScore < otherTeamScore) return 'text-red-500 dark:text-red-400';
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden p-6">
        <div className="flex justify-between items-center">
            <div className="text-center w-2/5">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">{team1?.unidade}</p>
            </div>
            <div className="text-center">
                 ...
                 <span className="text-2xl font-bold text-gray-400 dark:text-gray-500 mx-3">X</span>
            ...
            <div className="text-center w-2/5">
                 ...
                 <p className="text-sm text-gray-500 dark:text-gray-400">{team2?.unidade}</p>
            </div>
        </div>
        <div className="text-center mt-4 border-t border-gray-200 dark:border-gray-700 pt-4">
            <p className="text-gray-800 dark:text-gray-300 font-semibold">{confronto.description}</p>
            <p className="text-gray-400 dark:text-gray-500 text-sm mt-1">{new Date(confronto.date).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}</p>
        </div>
    </div>
  );
};
